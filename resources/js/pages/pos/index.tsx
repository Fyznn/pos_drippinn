...
export default function PosIndex({ auth, products }: PosProps) {
    ...
    const [lastTransaction, setLastTransaction] = useState({ total: 0, items: 0 }); // Simpan data sementara untuk struk
    ...
    const addToCart = (p: Product) => {
        ...
        if (exist) {
            if (exist.jumlah + 1 > p.stok) return alert('Stok Habis!');
    ...
    const processPayment = () => {
        ...
        // Simpan data transaksi untuk ditampilkan di modal sukses
        setLastTransaction({ total: grandTotal, items: cart.length });

        router.post(route('transaksi.store'), payload, {
            onSuccess: () => {
                setShowConfirmModal(false); // Tutup modal konfirmasi
                setCart([]); // Kosongkan keranjang
                setNamaPelanggan(''); // Reset nama
                setProcessing(false);
                
                // BUKA MODAL SUKSES (Gantikan Alert)
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="flex h-[calc(100vh-80px)] overflow-hidden font-sans bg-gray-50 relative">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                            {filtered.map(p => (
                                <div key={p.id} onClick={() => addToCart(p)} 
                                     className="group bg-white rounded-3xl p-4 cursor-pointer border border-transparent hover:border-indigo-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden"
                                >
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-50">
                                            ...
                                            <div className="h-8 w-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-400 group-hover:bg-indigo-600 group-hover:text-white transition-all">+</div>
                                        </div>
                                    </div>
                                </div>
                            ))}
                ...
                <div className="w-[400px] bg-white shadow-2xl flex flex-col h-full border-l border-gray-100 z-20 relative">
                    ...
                    <div className="p-8 bg-white border-t border-gray-100 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-10">
                        ...
                        <input 
                            ...
                            className="w-full mb-4 px-4 py-3 bg-gray-50 rounded-xl border-transparent focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all text-sm font-medium"
                            value={namaPelanggan}
                        ...
                        <button 
                            ...
                            className="w-full bg-gradient-to-r from-indigo-600 to-violet-600 text-white py-4 rounded-2xl font-bold text-lg shadow-lg shadow-indigo-200 hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                            💳 Bayar Sekarang
                        </button>
                    </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 relative z-10 animate-fade-in-up transform transition-all scale-100">
                            <div className="text-center">
                                ...
                                <h3 className="text-2xl font-black text-gray-900 mb-2">Konfirmasi Pembayaran</h3>
                                <div className="bg-gray-50 rounded-2xl p-6 mb-8 border border-gray-100 mt-6">
                                    <div className="flex justify-between items-center mb-2 text-gray-500 text-sm">
                                        <span>Total Item</span>
                                        <span className="font-bold">{cart.length} pcs</span>
                                    </div>
                                    <div className="flex justify-between items-center text-gray-900 text-xl font-black">
                ...
                {/* --- MODAL 2: SUKSES (PENGGANTI ALERT) --- */}
                {showSuccessModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 relative z-10 animate-bounce-in text-center">
                            ...
                            <h3 className="text-3xl font-black text-gray-900 mb-2">Berhasil!</h3>
                            <p className="text-gray-500 mb-8">Transaksi telah berhasil disimpan ke database.</p>
                            ...
                            <button 
                                ...
                                className="w-full py-4 px-6 bg-gray-900 text-white font-bold rounded-2xl shadow-xl hover:bg-gray-800 hover:scale-[1.02] transition-all"
                            >
                                Transaksi Baru ➜
                            </button>
                        </div>
